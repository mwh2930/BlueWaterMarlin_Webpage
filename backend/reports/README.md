...
**Deployment status — September 17, 2026.** The owner approved deployment and Git push. The isolated reader and upstream publisher are deployed, and the website's same-origin API connection is verified. Only Oregon Inlet is approved for lookup; its first current, scheduled report remains unverified. A lookup currently returns the generic missing-report 404, not sample data. Code and packaged-catalog defaults remain disabled. The website deployment workflow does not deploy this service.
...
This relay is deliberately request-driven and has no timer that pretends to regenerate data. The separate publisher is deployed and its timer is indexed; it must finish an atomic upload before the new version can be returned. The first current scheduled report still requires end-to-end verification. At a slot boundary, an older cached report immediately fails freshness; if the replacement has not finished publishing, the destination is unavailable. The short negative-cache interval can delay visibility of a newly available report by up to 15 seconds. Updates within the same publication slot can take up to the configured cache TTL to appear. Original report and source dates remain unchanged. Timer registration and enforced freshness do not prove that current inputs were exported or a report was published.
...
## Pilot verification and further rollout

1. Deployment, narrowly scoped identity assignments and native same-origin routing have been verified. The live catalog lists 76 candidates, with only Oregon Inlet approved for lookup. Direct backend access is denied; arbitrary or unapproved IDs return a generic 404.
2. Verify the first scheduled destination export and finished report against actual source timestamps. A working route and missing-object read do not establish report availability or current source coverage. Do not re-date historical inputs.
...
5. Keep other destinations unapproved until their inputs, provenance and generated reports have been reviewed. The separate staged cloud-fill wording draft remains unpublished and outside this release.
